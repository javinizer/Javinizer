function Set-EmbyActors {
    ...
    process {
        ...
        Write-Host "[$($MyInvocation.MyCommand.Name)] Getting actors from Emby"
        ...
        Write-Host "[$($MyInvocation.MyCommand.Name)] Importing R18 actors with thumb urls"
        ...
        foreach ($actor in $embyActors) {
            if ($actor.ImageTags -like $null) {
                if (($csvFullName -like $actor.Name) -or ($csvFullNameAlias -like $actor.Name)) {
                    ...
                    Write-Host "[$($MyInvocation.MyCommand.Name)] Writing thumburl [$thumbUrl] to actor [$($actor.Name)]"
                    Write-Debug "[$($MyInvocation.MyCommand.Name)] Performing [POST] on Uri [$embyServerUri/emby/Items/$actorId/RemoteImages/Download?Type=Thumb&ImageUrl=$thumbUrl&api_key=$embyApiKey]"
                    ...
                    Write-Debug "[$($MyInvocation.MyCommand.Name)] Performing [POST] on Uri [$embyServerUri/emby/Items/$actorId/RemoteImages/Download?Type=Primary&ImageUrl=$thumbUrl&api_key=$embyApiKey]"
