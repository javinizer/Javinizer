    ...
    process {
        ...
        foreach ($file in $nfoFiles) {
            ...
            if ($nfoNameString.Count -eq $nfoThumbString.Count) {
                ...
                $actressObject | Out-String | Write-Debug
                ...
                for ($x = 0; $x -lt $actressObject.Count; $x++) {
                    ...
                    if (($actressObject[$x].LastName -eq '' -and $actressObject[$x].FirstName -ne '') -and ($matched = Compare-Object -ReferenceObject ($actressCsv | Where-Object { $_.LastName -eq '' }) -DifferenceObject $actressObject[$x] -IncludeEqual -ExcludeDifferent -PassThru -Property @('FirstName'))) {
                        ...
                        if ($null -ne $matchedActress) {
                            ...
                            $actressString = $actressObject[$x] | ConvertTo-Json -Compress
                            Write-JVLog -Write:$script:JVLogWrite -LogPath $script:JVLogPath -WriteLevel $script:JVLogWriteLevel -Level Debug -Message "[$($MyInvocation.MyCommand.Name)] [Actress - $originalActressString] matched to [$actressString]"
                        }
                        ...
                        Write-JVLog -Write:$script:JVLogWrite -LogPath $script:JVLogPath -WriteLevel $script:JVLogWriteLevel -Level Debug -Message "[$($MyInvocation.MyCommand.Name)] [Actress - $($actressObject[$x] | ConvertTo-Json -Compress)] not matched"
                    }
                }
                ...
                $nfoContent | Out-File -LiteralPath $file -Force
            }

            $newNfoContent = (Get-Content -LiteralPath $file) -join "`r`n"
            if ($originalNfoContent -ne $newNfoContent) {
                Write-JVLog -Write:$script:JVLogWrite -LogPath $script:JVLogPath -WriteLevel $script:JVLogWriteLevel -Level Info -Message "Nfo had changes: [$file]"
            }
        }
    }
}
