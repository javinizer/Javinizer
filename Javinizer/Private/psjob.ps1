...
$files | Start-RSJob -VariablesToImport ScriptRoot -Verbose -ScriptBlock {
        $debugPreference = 'continue'
        $verbosePreference = 'continue'
        Javinizer -Path $_.FullName -DestinationPath ($_.Directory).tostring() -r18 -ScriptRoot $ScriptRoot -Verbose
    } -Throttle 3 -FunctionFilesToImport (Join-Path $PSScriptRoot -ChildPath 'Get-AggregatedDataObject.ps1'), `
